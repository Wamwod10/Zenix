...
// ✅ BACKEND INTEGRATION: kartani saqlash
// Backend faqat brand + oxirgi 4 raqamni saqlaydi, to'liq raqam va CVC saqlanmaydi
import { useSavePaymentCardMutation } from "../onboardingApi";
...
export default function PaymentCard() {
  ...
  // ✅ BACKEND INTEGRATION: POST /onboarding/payment-card
  const [savePaymentCard, { isLoading }] = useSavePaymentCardMutation();
  ...
  const handleSubmit = async (event) => {
    ...
    // ✅ BACKEND INTEGRATION: karta backend'da tekshiriladi (Luhn, muddat)
    // va xavfsiz saqlanadi (faqat brand + oxirgi 4 raqam)
    try {
      await savePaymentCard({
        cardNumber,
        holder: form.holder.trim(),
        expiry: form.expiry,
        ...(form.cvc ? { cvc: form.cvc } : {}),
    ...
    <main className="payment-card-page">
      <section className="payment-card-page__content">
        ...
        <form className="payment-card-page__form" onSubmit={handleSubmit}>
          ...
          >
            {isLoading ? "Tekshirilmoqda..." : "Sinovni boshlash"}
          </Button>
        </form>
      </section>

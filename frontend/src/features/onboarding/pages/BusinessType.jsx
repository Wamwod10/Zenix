...
// ✅ BACKEND INTEGRATION: biznes turini backend'ga saqlash
import { useSetBusinessTypeMutation } from "../onboardingApi";
...
export default function BusinessType() {
  ...
  // ✅ BACKEND INTEGRATION: PATCH /onboarding/business-type
  const [setBusinessType, { isLoading }] = useSetBusinessTypeMutation();

  // ✅ BACKEND INTEGRATION: tanlov bazaga saqlanadi, keyin davom etadi
  const handleContinue = async () => {
    if (!selectedCategory || isLoading) return;

    try {
      await setBusinessType({ businessType: selectedCategory.id }).unwrap();
